...
class MicroGridEnv(gym.Env):
    def __init__(self,**kwargs):
        ...
        self.battery = Battery(capacity=self.bat_capacity, useD=0.99, dissipation=0.001, rateC=0.99, maxDD=self.max_discharge, chargeE=self.max_charge)
    ...
    def _build_state(self):
        ...
        state = [avage_soc,loads,time_step,self.battery.SoC, current_generation,
                         price_grid_buy,temperature]
        return state
    ...
    def step(self, action, test=False):
        ...
        # print("tcl:",self._compute_tcl_power() * self.tcl_sale_price / 100)
        ...
        if avage_soc<=1 and avage_soc >= 0:
            ...
            reward += max(-8*(avage_soc-1), -1.5)
            self.reward_soc += max(-8*(avage_soc-1), -1.5)
        elif avage_soc < 0:
            reward += max(8*avage_soc, -1.5)
        ...
        if available_energy > 0:
            ...
            # print("电力不足:", self.grid.buy(self.energy_bought) / 80)
        ...
        if test == True:
            if terminal:
                ...
                #print( self.day,"comfortable",self.reward_soc,"tcl",self.reward_tcl,"sell",self.reward_sell-self.reward_buy,"load",self.reward_load,"avaible",self.elc_available_energy)
                #print( self.day,"pv",self.energy_pv,"tcl",self.energy_tcl,"load",self.energy_load,"sell",self.energy_sell,"buy",self.energy_buy,"battery",self.energy_battery)
                if self.day == 9:
                    ...
                    np.save("result/"+'baseline1_energy_sellnum.npy', np.array(REWAED_DAY))
                    np.save("result/"+'baseline1_energy_buynum.npy', np.array(ENERGY_DAY))
                    np.save("result/" + 'baseline1_total_sell.npy', np.array(REWAED_DAY) - np.array(ENERGY_DAY))
    ...
    def render(self, test_day, name='', display=[0,1,0,0]):
        ...
        if self.time_step == self.iterations:
            ...
            if display[1] == 1 or display[0] == 1:
                ...
                ax.boxplot(SOCS_RENDER, positions=range(24), patch_artist=True,
                           boxprops={'facecolor': "#1FA6BB"}, showfliers=False)
                print("Day:", test_day)

                plt.axhspan(19, 25, facecolor='#228B22', alpha=0.5)
                if test_day==0:
                    ax.set_ylabel("indoor temperature (°C)",fontdict={'size':15})
                # ax.set_facecolor("silver")
                # ax.yaxis.grid(True)
                ax.set_xlabel("Time (h)",fontdict={'size':15})
                positions = np.arange(1, 25)
                plt.yticks(np.arange(15, 30, step=2),)
                ax.boxplot(SOCS_RENDER, positions=positions,patch_artist=True,
                boxprops={'facecolor':"#1FA6BB"}, showfliers=False)
                print("dayday",self.day)
                ...
                # if test_day<5:
                #     plt.tight_layout()
                #     # plt.savefig('resultfig/DQN/cu_temper{}.pdf'.format(self.day) , format='pdf')
                #     np.save("result/" + 'baseline1_SOCS_RENDER{}.npy'.format(test_day), SOCS_RENDER)
                # # ax1 = ax.twinx()
                ...
                # # plt.legend(['GRID_PRICES_BUY','Energy allocated for TCLs', 'Energy consumed by TCLs'],loc='upper right')
                # # plt.xlabel("Time (h)")
                # # plt.ylabel("kW")
                # plt.show()
                #2 价格响应负载信息
            ...
            if display[3] == 1 or display[0] == 1:
                ...
                if test_day<10:
                    print("实验完成啦！！！！！！")
                    # plt.savefig('resultfig/DQN/baseline1_sell{}.pdf'.format(self.day), format='pdf')
                    np.save("result/" + 'baseline1_ENERGY_SOLD_RENDER{}.npy'.format(test_day), ENERGY_SOLD_RENDER)
                    np.save("result/" + 'baseline1_ENERGY_BOUGHT_RENDER{}.npy'.format(test_day), ENERGY_BOUGHT_RENDER)
                    plt.close()
                plt.savefig("result/"+'baseline1_ENERGY_SOLD_BOUGHT.svg', dpi=1500, bbox_inches='tight', format="svg")
        ...
        if test_day == 9 and self.time_step==self.iterations:
            ...
            plt.savefig("resultfig/baseline1_avg_soc.png", format='png')
            plt.show()
            np.save("result/" + 'baseline1_avg_soc.npy', AVG_SOC)
            print(AVG_SOC)
