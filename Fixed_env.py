...
class MicroGridEnv(gym.Env):
        ...
        if test == True:
            if terminal:
                ...
                if self.day == 9:
                    ...
                    np.save("result/"+'baseline2_energy_sellnum.npy', np.array(REWAED_DAY))
                    np.save("result/"+'baseline2_energy_buynum.npy', np.array(ENERGY_DAY))
                    np.save("result/"+'baseline2_total_sell.npy', np.array(REWAED_DAY) - np.array(ENERGY_DAY))
    ...
    def render(self,name='',display=[0,1,0,0]):
        ...
        if self.time_step==self.iterations:
            ...
            if display[1] ==1 or display[0] ==1:
                plt.rcParams.update({'font.size': 12})  # 设置默认字体大小
                #tcl的温度控制情况
                ...
                if self.day<3:
                    plt.tight_layout()
                    # plt.savefig('resultfig/baseline2_temper{}.pdf'.format(self.day), format='pdf')
                    np.save("result/" + 'baseline2_SOCS_RENDER{}.npy'.format(self.day), SOCS_RENDER)
                # ax1 = ax.twinx()
            ...
            if display[3] == 1 or display[0] == 1:
                ...
                ax.bar(x=np.array(np.arange(self.iterations)), height=np.array(ENERGY_SOLD_RENDER), color='navy',
                       width=0.8)
                ax.bar(x=np.array(np.arange(self.iterations)), height=np.array(ENERGY_BOUGHT_RENDER), color='darkred',
                       width=0.8)
                ax.set_xlabel("Time (h)")
            ...
            TEMP_RENDER.clear()
        if self.day == 9 and self.time_step==self.iterations:
            ma_rewards=[]
            fig = plt.axes()
            ...
            plt.show()
            np.save("result/" + 'baseline2_avg_soc.npy', AVG_SOC)
            print(AVG_SOC)
            # #####
